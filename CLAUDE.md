...
├── .mcp.json                          ← Screaming Frog MCP (Ahrefs is via claude.ai connector)
├── .claude/
│   ├── settings.local.json
...
│       └── {gsc-audit,keyword-cannibalization,sitemap-checking}(GSC)/  ← SKILL.md · analyzer .py
├── ahrefs/
...
└── screaming-frog/
    ...
    ├── CRAWL_DEFAULTS.md
    ├── scripts/{profile.js, manifest.js}
    ├── profiles/                      ← per-site crawl profiles
    ├── crawls/                        ← per-run NDJSON exports + manifest.json
...
2. **If the skill's tag contains `SF`**, **first** run the universal workflow in `screaming-frog/CLAUDE.md` (get URL → load/create profile → crawl or reuse → write manifest → resolve exports) for the SF parts. Skip this step if `SF` is not in the tag.
...
| `keyword-cannibalization(GSC)` | Detect keyword cannibalization from Google Search Console query→page data — flags queries where 2+ of the site's URLs compete. |
| `sitemap-checking(GSC)` | Cross-check XML sitemap URLs against GSC performance pages — flags sitemap URLs with no impressions (not-indexed / zero-impression candidates). |
| `gsc-audit(GSC)` | Full GSC checkup in one report — cannibalization, low-hanging-fruit, CTR opportunity, index hygiene, outdated content, sitemap indexing. |
...
- **Screaming Frog run identity:** `<domain-slug>/<run-id>` (e.g. `www-example-com/2026-06-11_09-30-00`) shared across `crawls/`, `reports/`, summary. Full rules → `screaming-frog/CLAUDE.md`.
...
- **Before first use of any Ahrefs MCP tool in a session**, call `mcp__claude_ai_Ahrefs__doc` for it.
...
4. If the new skill belongs to the Screaming Frog family and needs a profile field or a crawl export, declare it in frontmatter:
   ```yaml
   requires:
     profile: { near_duplicates: true }
     exports:
       - slug: my-export
         source: { type: bulk_export, name: "Some:Bulk Export" }
   ```
   The universal workflow's step 3 + 4a will check the profile and produce the export on demand.
