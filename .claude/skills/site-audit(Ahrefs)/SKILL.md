...
Wraps `mcp__claude_ai_Ahrefs__site-audit-*` tools into a raw → cleaned → summary pipeline. Family-wide rules live in `ahrefs/CLAUDE.md`.
...
2. **Ahrefs project identifier** — Ahrefs Site Audit runs against a pre-configured project in the user's Ahrefs account. Ask for the project's identifier (name or ID as it appears in `management-projects`). If unknown, call `mcp__claude_ai_Ahrefs__management-projects` to list available projects.
...
1. Call `mcp__claude_ai_Ahrefs__doc` for any `site-audit-*` tool before its first use this session.
2. Common picks:
